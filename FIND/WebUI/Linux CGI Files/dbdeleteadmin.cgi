...
my $button = $cgi->param( 'deleteFromDB' );
my $prevPage = $cgi->param( 'prevPage' );
my $nextPage = $cgi->param( 'nextPage' );

# Initialize Variable If Not Defined
$searchField = "Title" if !defined( $searchField );
$type = "Contains" if !defined( $type );
$startYear = 3000 if !defined( $startYear );
$endYear = 1900 if !defined( $endYear );
$results = 10 if !defined( $results );
...
ConnectToServer();
DeleteCheckItemsFromDatabase() if( defined( $button ) );
...
sub DeleteCheckItemsFromDatabase
{
    if( @checkedItems )
    {
        ...
        # Send Command Set PubMed Defaults (Sets Up Database Connection)
        $response = SendCommandReceiveMessage( $socket, "pubmeddefault $results" );
        
        # Send Command To Store Entries Into Database
        $response = SendCommandReceiveMessage( $socket, "dbdeleteentries $pmidStr" );
...
sub RetrieveDatabaseDataFromServer
{
    if( defined( $socket ) && $socket->connected() )
    {
        ...
        $dbEntryCount = SendCommandReceiveMessage( $socket, "dbentrycount" );
        ...
        $data = SendCommandReceiveMessage( $socket, "dbretrieveentries $retStart $results" ) if ( !defined( $keyword ) || $keyword eq "" );
        $data = SendCommandReceiveMessage( $socket, "dbretrieveentries $searchWords $searchBy $searchType $startYear $endYear $retStart $results" ) if ( defined( $keyword ) && $keyword ne "" );
...
sub BuildPageHeader{
	...
	<div class= "absolute">
	<form action = "dbdeleteadmin.cgi" method="get" onreset="formreset()">
...
if( $results )
{
	print <<"PAGEBODY";
            <div id ="section">
            <p><b> Search Results: </b> &nbsp &nbsp <font color="red">WARNING: VIEWING DATABASE DATA</font> <br>
...
print << "SELECTALL";
    <div style="margin-left:350px">
        <font color="red"> &nbsp &nbsp &nbsp &nbsp WARNING: VIEWING DATABASE DATA</font> &nbsp - &nbsp ($dbEntryCount Total Entries)
        <br>
	<input type="radio" name="select" onClick="selectall(this)" /> Select All 
	<input type="radio" name="select" onClick="deselectall(this)" /> Deselect All 
SELECTALL
...
print << "SUBMITRESULTS"
	<script language="JavaScript">
            ...
            <button name="deleteFromDB" type="submit" value="$retStart" style="margin-left:auto;margin-right:auto">Delete From Database</button>
	</form>
	</div>
	</body>
	</html>
...
else
{
    print <<"PAGEBODY";
    <div id ="section">
    <p><b> Search Results: </b> <font color="red"> &nbsp &nbsp WARNING: VIEWING DATABASE DATA</font></p>
PAGEBODY
