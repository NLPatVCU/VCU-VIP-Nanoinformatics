...
my $searchField = $cgi->param( 'field' );
...
my $button = $cgi->param( 'submitToDB' );
...
my @checkedItems = $cgi->param( 'checkbox' );
...
# Retain Current Index If "Submit To Database" Button Clicked
$retStart = $button if( defined( $button ) );
...
ConnectToServer();
SendQueryToServer();
AddCheckedItemsToDatabase() if( defined( $button ) );
RetrieveServerData();
...
sub SendQueryToServer
{
    if( defined( $socket ) && $socket->connected() )
    {
        # Setting Up Client-To-Server Variables
        $data = SendCommandReceiveMessage( $socket, "pubmeddefault $retStart $results" );
        
        # Fetch The Specified Query And Simultaneously Parse That Data (Quick Command)
        $data = SendCommandReceiveMessage( $socket, "queryandparsedata $keyword+AND+$endYear\[pdat]\:$startYear\[pdat]" );
        
        # Clear $data Variable Prior To Fetching Numeric Value
        $data = SendCommandReceiveMessage( $socket, "numofentries" );

        # If WebUI Requests Results Than There Entries, Correct WebUI Accordingly
        $results = $data if( looks_like_number( scalar( $data ) ) && scalar( $data ) < $results );
...
sub AddCheckedItemsToDatabase
{
    if( @checkedItems )
    {
    	# Concatenate PMIDs To Store In Database
        my $pmidStr = join( ':', @checkedItems );
        
        # Send Command To Store Entries Into Database
        $response = SendCommandReceiveMessage( $socket, "dbstoreentries $pmidStr" );
        
        # Clear "Button" Variable To Prevent Running This Function Again Next Script Iteration
        $button = "";
        
        # Clears Server EUtilities Data
        $response = SendCommandReceiveMessage( $socket, "cleareutil" );
        
        # Clears Server PubMedXMLParser Data
        $response = SendCommandReceiveMessage( $socket, "clearparser" );

        # Send Query Again To Fetch New Data Not Already Existing In Database
        SendQueryToServer();
    }
}

sub RetrieveServerData
{
    ################################################################################################
    #       Begin Data Fetching And Client Side Parsing (Store Data In Global Arrays)
    ################################################################################################
    ...
    {   
        ...
        {
            $response = SendCommandReceiveMessage( $socket, "get element $index alldata" );
            ParseEntryData( $response );
        }
        
        # Clear Received Data Prior To Loop Interation
        $data = "";
        $response = "";
    }
}
...
sub BuildPageHeader
{
    ...
    <div class= "absolute">
    <form action = "results_admin.cgi" method="get" onreset="formreset()">
...
{	
    ...
    <div id = "nav">
            ...
            <br>
            &nbsp &nbsp &nbsp
            <a href="dbdeleteadmin.cgi">
                View Database Data
            </a>
            <br>
            &nbsp &nbsp &nbsp
            <a href="../FIND_admin.html">
                Back To Home Page
            </a>
    </div>
PAGENAV
...
if( $results )
{
    print <<"PAGEBODY";
        ...
        <p><b> Search Results: </b><br>
                <input type="radio" name="select" onClick="selectall(this)" /> Select All 
                <input type="radio" name="select" onClick="deselectall(this)" /> Deselect All 
        </p>
        <!-- <form action = ""> -->
PAGEBODY
...
SCRIPT
	...
	print << "SEARCHRESULTS1";
            <div class = "result">
            <input type = "checkbox" value="$pmid[$i-1]" name = "checkbox">$index.<b> $articleTitle[$i-1]</b>
...
SEARCHRESULTS2
	...
	<tr>
		...
		<td >
			<a  href = "http://www.pubmed.com"> PubMed</a>
		</td>
	</tr>
	</table>
	</ul>
...
print << "SUBMITRESULTS"
	<script language="JavaScript">
            ...
            <button name="submitToDB" type="submit" value="$retStart" style="margin-left:auto;margin-right:auto">Submit To Database</button>
	</form>
	</div>
	</body>
	</html>
